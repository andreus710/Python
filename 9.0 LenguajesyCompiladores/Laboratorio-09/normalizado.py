...
class Tabla:
    ...
    def cargar_desde_excel(self, archivo_excel):
        ...
        try:
            ...
            for idx, (estado, fila) in enumerate(df.iterrows(), 1):
                ...
                self.M[idx][0] = str(int(estado)) if isinstance(estado, float) else str(estado)  # Convertir estado a entero si es float
                
                for j, valor in enumerate(fila[:NUM_TABLE_COLS-1], 1):
                    if pd.notna(valor):
                        # Convertir valores numéricos flotantes a enteros cuando sea apropiado
                        valor_str = str(valor).strip()
                        if valor_str.replace('.0', '').isdigit() and valor_str.endswith('.0'):
                            valor_str = valor_str.replace('.0', '')
                        self.M[idx][j] = valor_str
                    else:
                        self.M[idx][j] = " "
            
            print(f"Tabla cargada exitosamente desde archivo Excel")
    ...
    def cargar_tabla_por_defecto(self):
        """Carga una tabla por defecto basada en los datos del PDF"""
        # Inicializar matriz
        self.M = [[" " for _ in range(NUM_TABLE_COLS)] for _ in range(NUM_TABLE_ROWS)]
        
        # Encabezados
        headers = ["Estado", "switch", "case", "break", "default", "(", ")", "{", "}", "..", ";", ":", "id", "num", "$", "S", "A", "M", "B", "T", "C", "D"]
        for i, header in enumerate(headers):
            if i < NUM_TABLE_COLS:
                self.M[0][i] = header
        
        # Datos de la tabla basados en el PDF
        tabla_data = {
            0: {"switch": "d3", "S": "1", "A": "2"},
            1: {"$": "accept"},
            2: {"switch": "d6", "M": "5", "$": "4"},
            ...
            11: {"{": "d13"},
            12: {"num": "d14"},
            13: {"case": "d16", "default": "r6", "}": "r6", "B": "15"},
            14: {"case": "d16", "default": "r6", "}": "r6", "B": "17"},
            15: {"default": "d19", "}": "r12", "D": "18"},
            16: {"num": "d20"},
            17: {"default": "d19", "}": "r12", "D": "21"},
            18: {"}": "d22"},
            19: {":": "d23"},
            20: {"..": "d25", ":": "r8", "T": "24"},
            21: {"}": "d26"},
            22: {"switch": "r2", "$": "r2"},
            23: {"break": "d28", ";": "r10", "C": "27"},
            24: {":": "d29"},
            25: {"num": "d30"},
            26: {"switch": "r2"},
            27: {"default": "r11"},
            28: {";": "d31"},
            29: {"switch": "d34", "case": "d34", "default": "d34", "}": "r4", "A": "33", "M": "32"},
            30: {":": "r7"},
            31: {"switch": "r9"},
            32: {"switch": "d36", "case": "d36", "default": "d36", "}": "r10", "B": "35"},
            33: {"switch": "r3", "case": "r3", "default": "r3", "}": "r3"},
            34: {"(": "d37"},
            35: {"case": "d16", "default": "r6", "}": "r6", "B": "38"},
            36: {"num": "d39"},
            37: {"id": "d40"},
            38: {"default": "r5", "}": "r5"},
            39: {"..": "r9", ":": "r9", "T": "r9"},  # Verificar esta entrada
            40: {")": "d41"},
            41: {"{": "d42"},
            42: {"case": "d16", "default": "r6", "}": "r6", "B": "43"},
            43: {"default": "d19", "}": "r12", "D": "44"},
            44: {"}": "d45"},
            45: {"switch": "r2", "case": "r2", "default": "r2", "}": "r2"}
        }
        
        # Llenar la tabla
        for estado, acciones in tabla_data.items():
            if estado < NUM_TABLE_ROWS:
                self.M[estado][0] = str(estado)
                for simbolo, accion in acciones.items():
                    # Encontrar la columna del símbolo
                    for j in range(1, NUM_TABLE_COLS):
                        if self.M[0][j] == simbolo:
                            self.M[estado][j] = accion
                            break
    ...
    def mostrar_tabla(self):
        ...
        for i in range(1, NUM_TABLE_ROWS):
            if self.M[i][0] != " ":  # Solo mostrar filas con datos
...
class Parser:
    ...
    def empilar(self, simbolo):
        if len(self.pila) < TOTALPILA:
            # Normalizar valores antes de apilar
            if isinstance(simbolo, float) and simbolo.is_integer():
                simbolo = str(int(simbolo))
            else:
                simbolo = str(simbolo)
    ...
    def long_produccion(self, num_regla_str):
        try:
            regla_num = int(float(num_regla_str))  # Manejar posibles valores flotantes
